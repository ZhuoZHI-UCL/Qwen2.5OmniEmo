...
def _build_noop_dummy_batch(tokenizer, length: int = 2) -> dict[str, torch.Tensor]:
    ...
    return {
        ...
        "labels": torch.full((1, length), IGNORE_INDEX, dtype=torch.long),
    }
...
class MultiModalDataCollatorForSeq2Seq(DataCollatorForSeq2Seq):
    ...
    #对call 函数加上了异常捕获
    def __call__(self, features: list[dict[str, Any]]) -> dict[str, "torch.Tensor"] | None:
        try:
            batch_images, batch_videos, batch_audios = [], [], []
            batch_imglens, batch_vidlens, batch_audlens, batch_input_ids = [], [], [], []
            for feature in features:
            ...
            # 一把抓：任何异常都跳过该 batch
            print(f"[Warning] Collator caught exception, skipping batch: {e}")
            return _build_noop_dummy_batch(self.tokenizer, 2)
...
class SFTDataCollatorWith4DAttentionMask(MultiModalDataCollatorForSeq2Seq):
    ...
    def __call__(self, features: list[dict[str, Any]]) -> dict[str, "torch.Tensor"]:
        features = super().__call__(features)
         ...
         # ===== 新增兜底：父类可能在异常/空 position_ids 时返回 None；这里直接构造一个 no-op batch 返回 =====
        if features is None:
            print("[Warning] Collator got None from parent (skipping problematic batch). Returning a no-op dummy batch.")
            # 选择一个安全的 token 作为占位：优先 pad_token_id，其次 eos_token_id，最后 0
            ...
            dummy = {
                ...
                "attention_mask": torch.tensor([[0]], dtype=torch.long),          # 0 表示不参与；也可用 1，但 label 全 IGNORE_INDEX
                "labels": torch.tensor([[IGNORE_INDEX]], dtype=torch.long),
            }
            # 注意：不返回 position_ids/rope_deltas 等，模型会把这个 batch 当成几乎“空”的样本，不贡献 loss
            return dummy
