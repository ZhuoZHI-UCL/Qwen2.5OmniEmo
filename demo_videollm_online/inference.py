...
class LiveInfer:
    ...
    def _call_for_streaming(self, ):
        while self.frame_embeds_queue:
            ...
            video_time, frame_embeds = self.frame_embeds_queue.popleft() #取一帧
            if not self.past_key_values: #首帧 or 上次是 EOS → 准备好起始/追加 prompt
            ...
            inputs_embeds = torch.cat([ #构造本步输入：仅追加“最后一个 token”的嵌入 + 本帧嵌入
            ...
            if next_score[:,:,self.frame_token_interval_id] < self.frame_token_interval_threshold: #注意，这个时候才进行回复哈
    ...
    def input_query_stream(self, query, history=None, video_time=None):
        if video_time is None: #这个不用管，因为我们只问一个问题而且只问一遍哈
    ...
    def input_video_stream(self, video_time):
        '''
        把“新出现”的帧区间编码为视觉嵌入并入队，不重复处理旧帧。
        visual_embed 是模型内置视觉编码（通常 CLIP 变体 + MLP 映射到 LLM 词向量空间）。
        每帧拆成 frame_num_tokens 个视觉 token，随带该帧的时间戳一起入队（用于时间对齐）。
        '''
    ...
    def load_video(self, video_path): #一次性把视频帧读到 GPU（TCHW），减少后续拷贝。
