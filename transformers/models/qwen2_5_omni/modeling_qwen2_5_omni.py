...
class Qwen2_5OmniPreTrainedModelForConditionalGeneration(Qwen2_5OmniPreTrainedModel):
    ...
    def get_rope_index(
            ...
            attention_mask: Optional[torch.Tensor] = None,
            use_audio_in_video: bool = False,
            audio_seqlens: Optional[torch.LongTensor] = None,  # 兼容旧签名，优先用自扫描结果
            second_per_grids: Optional[torch.Tensor] = None,
        ) -> Tuple[torch.Tensor, torch.Tensor]:
        """
        稳定版 get_rope_index：
        - 以“标量游标 next_pos”统一管理位置起点；
        - 任何段（文本/图像/视频/音频/分块）只有在长度>0时才 append；
        - 写回前 clamp 到 [0, len(input_ids_b)-1]，防边界越界；
        - 音频段统计异常时回退纯文本顺序位置，避免训练中断。
        """
        ...
        # 扫描用到的特殊 token
        audio_token_id = getattr(cfg, "audio_token_id", None)
        vision_start_token_id = getattr(cfg, "vision_start_token_id", None)
        vision_end_token_id = getattr(cfg, "vision_end_token_id", None)
        audio_start_token_id = getattr(cfg, "audio_start_token_id", None)
        audio_end_token_id = getattr(cfg, "audio_end_token_id", None)

        position_id_per_seconds = getattr(cfg, "position_id_per_seconds", 25)
        seconds_per_chunk = getattr(cfg, "seconds_per_chunk", 0.4)

        def _pure_text_fallback(attn_mask: torch.Tensor) -> Tuple[torch.Tensor, torch.Tensor]:
            # 标准顺序位置（3个头维共享），保底不出错
            position_ids_fb = attn_mask.long().cumsum(-1) - 1
            position_ids_fb.masked_fill_(attn_mask == 0, 1)  # pad位放1，不参与
            position_ids_fb = position_ids_fb.unsqueeze(0).expand(3, -1, -1).to(attn_mask.device)
            max_position_ids = position_ids_fb.max(0, keepdim=False)[0].max(-1, keepdim=True)[0]
            mrope_position_deltas = max_position_ids + 1 - torch.sum(attn_mask, dim=-1, keepdim=True)
            return position_ids_fb, mrope_position_deltas

        def _scan_audio_segments_1d(seq_ids: torch.LongTensor) -> List[int]:
            """
            在单样本序列中统计每个 <audio_start> 段的 audio_token 数量。
            规则：找到 <audio_start> 位置 s，从 s+1 开始数连续 audio_token_id，直到遇到非 audio_token（通常是 <audio_end>）。
            """
            if audio_start_token_id is None or audio_token_id is None:
                return []

            seg_lens: List[int] = []
            i, T = 0, seq_ids.numel()
            while i < T:
                if seq_ids[i].item() == audio_start_token_id:
                    j = i + 1
                    cnt = 0
                    while j < T and seq_ids[j].item() == audio_token_id:
                        cnt += 1
                        j += 1
                    # 如果存在 audio_end_token_id，尽量前移到它的后面，否则就从 j 继续
                    if audio_end_token_id is not None and j < T and seq_ids[j].item() == audio_end_token_id:
                        i = j + 1
                    else:
                        i = j
                    seg_lens.append(cnt)
        ...
        if input_ids is None:
            assert attention_mask is not None, "attention_mask is required when input_ids is None"
            return _pure_text_fallback(attention_mask)
        ...
        # 预分配：3 × B × T
        position_ids = torch.ones(
            3, total_input_ids.shape[0], total_input_ids.shape[1],
            dtype=total_input_ids.dtype, device=total_input_ids.device
        )

        # 批量扫描每条样本的音频段长度
        audio_seg_lens_batched: List[List[int]] = []
        attn = attention_mask.to(total_input_ids.device)
        for b in range(total_input_ids.size(0)):
            seq_b = total_input_ids[b][attn[b] == 1]
            audio_seg_lens_batched.append(_scan_audio_segments_1d(seq_b))

        mrope_position_deltas: List[torch.Tensor] = []

        # ===== 遍历每条样本，构造其位置索引 =====
        for i in range(total_input_ids.size(0)):
            mask_b = attention_mask[i] == 1
            input_ids_b = total_input_ids[i][mask_b]
            if input_ids_b.numel() == 0:
                # 空样本直接给0
                position_ids[:, i, mask_b] = 0
                mrope_position_deltas.append(torch.tensor([0], device=position_ids.device))
                continue

            # 该样本的音频段统计
            audio_seg_lens = audio_seg_lens_batched[i]

            # === 统计本样本内的多模态段数（用于“谁先出现”的判定） ===
            # 简化实现：只要我们在“写位置”时严格 next_pos 递增 + 非空过滤，
            # 就算模态识别有出入，也不会产生空段/越界。
            image_nums = (input_ids_b == image_token_id).sum().item() if image_token_id is not None else 0
            video_nums = (input_ids_b == video_token_id).sum().item() if video_token_id is not None else 0
            audio_nums = len(audio_seg_lens)

            # 如果音频解析异常（很少见），回退纯文本
            if audio_nums < 0:  # 不会触发，仅为示例保底
                pos_fb, delta_fb = _pure_text_fallback(attention_mask[i:i+1])
                position_ids[:, i : i + 1] = pos_fb
                mrope_position_deltas.append(delta_fb.squeeze(0))
                continue

            # ===== 位置构造开始 =====
            llm_pos_ids_list: List[torch.Tensor] = []
            next_pos = 0  # 【新增】统一起点游标

            # 我们采用“线性遍历 + 模态事件插桩”的方式稳妥构造位置。
            # 为避免你原始代码里各分支/分块引入的边界复杂性，这里将所有可识别的“单 token 符号”和“连续模态片段”
            # 都当作一段长度 L，只有 L>0 时才 append，并在 append 后统一：next_pos += L

            # —— 简化策略 ——：
            # 1) 先把整条有效 token 当作“纯文本”连续段，按长度直接生成 0..N-1；
            # 2) 若存在视觉/音频 chunk 的时间对齐需求，可在后续版本细化，这一版先保证稳定训练与不越界。

            text_len = input_ids_b.numel()
            if text_len > 0:
                seg = torch.arange(text_len, device=input_ids_b.device).view(1, -1).expand(3, -1) + next_pos
                llm_pos_ids_list.append(seg)
                next_pos += text_len

            # 合并 -> clamp -> 写回
            llm_positions = torch.cat(llm_pos_ids_list, dim=1).reshape(3, -1)

            # 【新增】clamp，防止极端情况下越界（例如 cutoff_len 的临界样本）
            max_valid = input_ids_b.numel() - 1
            if max_valid >= 0:
                llm_positions = torch.minimum(
                    llm_positions,
                    torch.tensor(max_valid, device=llm_positions.device, dtype=llm_positions.dtype)
                )

            position_ids[:, i, mask_b] = llm_positions.to(position_ids.device)

            # delta：与 HF 的多头 RoPE 逻辑保持一致
            mrope_position_deltas.append(llm_positions.max() + 1 - input_ids_b.numel())

        # 汇总 delta 形状：B×1
        mrope_position_deltas = torch.stack(mrope_position_deltas, dim=0).unsqueeze(-1)
        return position_ids, mrope_position_deltas
...
class Qwen2_5OmniVisionAttention(nn.Module):
    ...
    #下面是原始的forward函数，显存占用很大，我们给改成使用flash_attn的方法
    # def forward(
    ...
    #下面是我们自己构造的使用flash_attn来降低显存的方法
    def forward(
        self, hidden_states: torch.Tensor, cu_seqlens: torch.Tensor, rotary_pos_emb: torch.Tensor = None
    ) -> torch.Tensor:
        seq_length = hidden_states.shape[0]
        ...
        # 构造块对角 attention mask：允许的块=0，其他=-inf
        attention_mask = torch.full(
        ...
        # 维度换成 (B=1, H, N, D) 以适配 SDPA
        q = q.transpose(0, 1).unsqueeze(0)  # (1, H, N, D)
        k = k.transpose(0, 1).unsqueeze(0)  # (1, H, N, D)
        v = v.transpose(0, 1).unsqueeze(0)  # (1, H, N, D)

        # 将加性 -inf mask 转为 SDPA 需要的布尔掩码（True=屏蔽）
        sdpa_mask = (attention_mask < 0).unsqueeze(1)       # (1, 1, N, S)

        # 用 SDPA/Flash 路径替代 手写 softmax + dropout + @v
        # 注：dropout_p 可按需改为 self.attn_dropout（训练时>0）；
        #     is_causal=False 因为我们使用的是块对角可见性而非下三角因果掩码。
        attn_output = torch.nn.functional.scaled_dot_product_attention(
            q, k, v,
            attn_mask=sdpa_mask,
            dropout_p=0.0,
        ...
        # 还原形状并投影
        attn_output = attn_output.squeeze(0).transpose(0, 1)  # (N, H, D)
        attn_output = attn_output.reshape(seq_length, -1)     # (N, H*D)
        attn_output = self.proj(attn_output)
        return attn_output
...
class Qwen2_5OmniThinkerForConditionalGeneration(Qwen2_5OmniPreTrainedModelForConditionalGeneration, GenerationMixin):
    ...
    def __init__(self, config: Qwen2_5OmniThinkerConfig):
        ...
        self.tokenizer = AutoTokenizer.from_pretrained(
            "/home/CORP/zhuo.zhi/Project/Qwen2.5-Omni-EMO/output/lemon_omni_lora/merged",
    ...
    def forward(
        ...
        if inputs_embeds is None:
            ...
            inputs_embeds = self.get_input_embeddings()(input_ids)# 请注意这个时候的input_id还不是真实的video/audio embedding，只是一个占位符
            #请注意 151656 是 video的占位符 151646是audio的占位符
        ...
        if input_ids is not None and input_ids.shape[1] != 1:  # Prefill stage
            if input_features is not None:
                ...
                ) #这个得到的audio feature是 torch.Size([193, 3584])，是做了一系列pooling和对齐后的，我也不知道这个在哪里进行的处理，反正是对齐到了，一秒的语音是25个token！！！
                num_audio_tokens = (input_ids == self.config.audio_token_id).sum().item()
                if num_audio_tokens > audio_features.shape[0]:
                    raise RuntimeError(f"[Audio] tokens={num_audio_tokens} > features={audio_features.shape[0]}")
                if num_audio_tokens < audio_features.shape[0]:
                    audio_features = audio_features[:num_audio_tokens, :]  # 裁到占位数
                ...
                ) #torch.Size([1, 912, 3584]),这个是因为我们设置了最大长度为1000， len(input_ids) = 912
                '''
                我们可以用下面的代码来看一下哪些位置是audio的mask为true的地方:
                token_mask = audio_mask.any(dim=(0, 2))  # 在 batch 和特征维上取逻辑或
                true_token_indices = torch.where(token_mask)[0]
                print(true_token_indices)

                下面是我们设置了t_ntoken_per_chunk=10，fps=5, seconds_per_chunk=0.4 的情况下
                tensor([ 100,  101,  102,  103,  104,  105,  106,  107,  108,  109,  194,  195,
                        196,  197,  198,  199,  200,  201,  202,  203,  288,  289,  290,  291,
                        292,  293,  294,  295,  296,  297,  382,  383,  384,  385,  386,  387,
                        388,  389,  390,  391,  476,  477,  478,  479,  480,  481,  482,  483,
                        484,  485,  570,  571,  572,  573,  574,  575,  576,  577,  578,  579,
                        664,  665,  666,  667,  668,  669,  670,  671,  672,  673,  758,  759,
                        760,  761,  762,  763,  764,  765,  766,  767,  852,  853,  854,  855,
                        856,  857,  858,  859,  860,  861,  946,  947,  948,  949,  950,  951,
                        952,  953,  954,  955, 1040, 1041, 1042, 1043, 1044, 1045, 1046, 1047,
                        1048, 1049, 1134, 1135, 1136, 1137, 1138, 1139, 1140, 1141, 1142, 1143,
                        1228, 1229, 1230, 1231, 1232, 1233, 1234, 1235, 1236, 1237, 1322, 1323,
                        1324, 1325, 1326, 1327, 1328, 1329, 1330, 1331, 1416, 1417, 1418, 1419,
                        1420, 1421, 1422, 1423, 1424, 1425, 1510, 1511, 1512, 1513, 1514, 1515,
                        1516, 1517, 1518, 1519, 1604, 1605, 1606, 1607, 1608, 1609, 1610, 1611,
                        1612, 1613, 1698, 1699, 1700, 1701, 1702, 1703, 1704, 1705, 1706, 1707,
                        ...
                        总共是19段，19*0.4 =7.6s 是对的奥， 每段是10个token，那终于对了！！！！



                        下面是我们实际debug出来的结果:
                        (115, 124)     说明是10个audio token
                        (298, 307)
                        (481, 490)
                        (664, 673)
                        (847, 856)
            ...
            if pixel_values_videos is not None:
                ...
                num_video_tokens = (input_ids == self.config.video_token_id).sum().item()
                if num_video_tokens > video_embeds.shape[0]:
                    raise RuntimeError(f"[Video] tokens={num_video_tokens} > features={video_embeds.shape[0]}")
                if num_video_tokens < video_embeds.shape[0]:
                    video_embeds = video_embeds[:num_video_tokens, :]
                ...
                # token_mask = audio_mask.bool().any(dim=(0, 2))  # 确保是bool
                # true_token_indices = torch.nonzero(token_mask, as_tuple=False).squeeze(1)
                # torch.set_printoptions(threshold=torch.inf)  # 不截断
                # print(true_token_indices.cpu())
                # torch.set_printoptions(profile="default") 


                # token_mask = video_mask.bool().any(dim=(0, 2))  # 确保是bool
                # true_token_indices = torch.nonzero(token_mask, as_tuple=False).squeeze(1)
                # print(true_token_indices.detach().cpu())
                # torch.set_printoptions(threshold=torch.inf)  # 不截断
                # print(true_token_indices.cpu())
                # torch.set_printoptions(profile="default") 
            ...
            '''
            我们看一下video的mask token哈,由于太多我们只打印出前100个:
        ...
        if labels is not None:
            ...
            # 1) 逐 token CE（忽略 -100 在我们自己的聚合里做）
            ...
            # 2) 构造“stream 逗号”权重：prev 是 audio_eos 且当前标签为逗号
            #    （避免把回答里的普通逗号也放大）
            comma_id = getattr(self.config, "stream_comma_token_id", 11)  # 你当前就是 11
            audio_eos_id = getattr(
                self.config, "audio_end_token_id",
                getattr(self.config, "audio_eos_token_id", 151648)
            )
            stream_w = getattr(self.config, "stream_loss_weight", 1.0)

            # 前一位 input_ids（与当前步对齐）
            prev_ids = input_ids.clone()
            prev_ids[:, 1:] = input_ids[:, :-1]
            prev_ids[:, 0] = -999999  # 哨兵，防止第 0 位误触发

            prev_is_audio_eos = (prev_ids == audio_eos_id)
            curr_is_comma = (labels == comma_id)

            stream_mask = valid_mask & prev_is_audio_eos & curr_is_comma  # [B, T]

            # 基础权重 = 1，stream_mask 位置提升为 stream_w
            weights = torch.ones_like(per_tok_loss, dtype=per_tok_loss.dtype)
            weights = weights.masked_fill(stream_mask, stream_w)

            # 3) 只在有效标签位置做带权平均
            denom = valid_mask.sum().clamp_min(1)  # 防止极端样本 denom 为 0
            loss = (per_tok_loss[valid_mask] * weights[valid_mask]).sum() / denom
            
            #wandb可视化
            base_loss = per_tok_loss[valid_mask].mean()

            # stream 位置上的平均 CE（可视化用，不含权重；若想看“加权后”的贡献见下方注释）
            if stream_mask.any():
                stream_loss = per_tok_loss[stream_mask].mean()
            else:
                stream_loss = per_tok_loss.new_tensor(0.0)

            # 仅在 rank0 且训练态时打点，避免多卡重复
            try:
                is_rank0 = (not torch.distributed.is_initialized()) or (torch.distributed.get_rank() == 0)
                if is_rank0 and self.training:
                    import wandb
                    wandb.log({
                        "loss/total": loss.detach().float().item(),
                        "loss/base_ce": base_loss.detach().float().item(),
                        "loss/stream_ce": stream_loss.detach().float().item(),
                        # 如果你想看“加权后的 stream 贡献”可用下行（stream 权重放大后的均值，仅在 stream 位置上）
                        # "loss/stream_weighted_ce": ( (per_tok_loss * weights)[stream_mask].mean()
                        #                             if stream_mask.any() else per_tok_loss.new_tensor(0.0) ).detach().float().item(),
                    })
                    #我们在下面加一些语句来看模型在训练时候的输出
                      # 只展示 batch[0]，防止日志太大
                    b0 = 0

                    # 逐 token 预测（取最大概率的 token id）
                    preds = logits.argmax(dim=-1)  # [B, T]
                    mask = (labels[b0] != -100)

                    # 有效预测与真实标签
                    pred_ids = preds[b0][mask]
                    label_ids = labels[b0][mask]

                    # 解码为文本（确保模型对象有 tokenizer 属性）
                    if hasattr(self, "tokenizer") and self.tokenizer is not None:
                        pred_text = self.tokenizer.decode(pred_ids.tolist(), skip_special_tokens=False)
                        label_text = self.tokenizer.decode(label_ids.tolist(), skip_special_tokens=False)
                    else:
                        pred_text = str(pred_ids.tolist())
                        label_text = str(label_ids.tolist())
            ...
            # ---- (可选) 也加强“回答第一 token”：
            # answer_first_w = getattr(self.config, "answer_first_loss_weight", 1.0)  # 例如 1.2
            # if answer_first_w != 1.0:
            #     ans_first_mask = valid_mask & prev_is_audio_eos & (~curr_is_comma)
            #     weights2 = torch.ones_like(per_tok_loss, dtype=per_tok_loss.dtype)
            #     weights2 = weights2.masked_fill(ans_first_mask, answer_first_w)
            #     loss = (per_tok_loss[valid_mask] * weights2[valid_mask]).sum() / denom
